...
bool WifiManager::open_access_point() {
	...
	// Access point name
	char ap_ssid[64];
	sprintf(ap_ssid, "ap-%s-%s", config.dev_id, config.node_id);

	// Configure WiFi for soft AP 
	WiFi.disconnect();
	WiFi.mode(WIFI_AP);

	if (!WiFi.softAPConfig(ap_address, ap_address, IPAddress(255, 255, 255, 0))) {
		return false;
	}
	if (!WiFi.softAP(ap_ssid, CONFIG_PORTAL_PASS)) {
		return false;
	}
	delay(500);
